...
PRED_FILE <- file.path(OUTDIR_PRED, "mcaware_BiLSTM_v3THYAO_PREDICTIONS.csv")
...
# Kaydet
write.csv(label_check, file.path(OUTDIR_DIAG, "mcaware_v4_LABEL_CHECK.csv"),
          row.names = FALSE)
...
if (!is.null(mi_df) && nrow(mi_df) > 0) {
  ...
  # Kaydet
  write.csv(mi_df, file.path(OUTDIR_DIAG, "mcaware_v4_MI_SCORES.csv"),
            row.names = FALSE)
