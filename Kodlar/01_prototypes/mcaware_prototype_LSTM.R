...
# T�B�TAK 2209-A �n Kan�t �al��mas� (Mehmet Ali Kurt, y�r�t�c�)
# ---------------------------------------------------------------------------
# Ama�:
#   AMZ LSTM full/In=2/Out=3 �ampiyon konfig�rasyonu �zerinde MC_Penalty
#   teriminin etkisini �l�mek.
#
# Kar��la�t�rma:
#   Vanilla BCE  vs  BCE + ?�|mean(y)-0.5|   (? ? {0.0, 0.1, 0.3, 0.5})
#   3 seed: 23, 27, 98 (mevcut �al��mayla ayn�)
#
# �al��t�rma:
#   1) RStudio'da bu dosyay� a�.
#   2) setwd() yolunu kontrol et (mevcut EMEKLILIK_GUNCEL.R ile ayn�).
#   3) Source > Run All
#   4) ��kt�: mcaware_LSTM_RESULTS.csv (�zet) + konsol tablosu
#
# S�re: CPU modunda yakla��k 30-60 dakika (12 ko�u � ~3-5 dakika)
# ===========================================================================
...
# --- 0. Ortam ---
setwd(here::here())  # senin EMEKLILIK_GUNCEL.R'daki ile ayn�
...
# --- 2. �zellik m�hendisli�i (full set, TEFAS proxies) ---
...
amz$Volatility <- zoo::rollapply(c(NA, diff(log(amz$Close))),
                                  ...
                                  fill = NA, align = "right")
amz <- amz %>% drop_na()
cat(sprintf("AMZ veri (warmup sonras�): %d hafta\n", nrow(amz)))
...
for (t in (IN_LEN + 1):(N - OUT_LEN)) {
  ...
  # Yukar�daki: t = mevcut nokta. Pencere = t-IN_LEN .. t-1 (ge�mi� IN_LEN hafta).
...
y_arr <- y_vec
cat(sprintf("Toplam pencere: %d | s�n�f Up=%%%.1f\n",
...
# --- 7. Model fabrikas� (AMZ �ampiyonu HP: adam / tanh / dropout=0.4) ---
build_lstm <- function(seed, lambda_mc = 0.0) {
  ...
  # class weight (AMZ s�n�f dengesizli�i i�in)
...
for (lam in LAMBDAS) {
  for (sd_ in SEEDS) {
    cat(sprintf("Ko�uluyor: ?=%.1f seed=%d ...\n", lam, sd_))
...
cat("\n", strrep("=", 85), "\n", sep = "")
cat("�ZET � ? ba��na 3-seed ortalamas�\n")
cat(strrep("=", 85), "\n", sep = "")
...
cat("Bu CSV'leri benimle payla��rsan strateji dosyas�na '�n kan�t' olarak i�leyece�im.\n")
