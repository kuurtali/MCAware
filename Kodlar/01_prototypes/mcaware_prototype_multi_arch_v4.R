...
if (!dir.exists(OUTDIR)) { warning("OUTDIR yok, WORKDIR kullaniliyor"); OUTDIR <- WORKDIR }
...
for (arch_name in names(ARCH_BUILDERS)) {
  ...
  write.csv(pred_df, file.path(OUTDIR, paste0(prefix,"PREDICTIONS.csv")), row.names=FALSE)
  write.csv(thr_df,  file.path(OUTDIR, paste0(prefix,"THRESHOLD_GRID.csv")), row.names=FALSE)
  write.csv(opt_df,  file.path(OUTDIR, paste0(prefix,"OPTIMAL.csv")), row.names=FALSE)
  write.csv(yhat_df, file.path(OUTDIR, paste0(prefix,"YHAT_STATS.csv")), row.names=FALSE)
...
write.csv(cross_df, file.path(OUTDIR, "mcaware_multi_arch_CROSS_ARCH_SUMMARY.csv"),
          row.names=FALSE)
...
all_preds_file <- file.path(OUTDIR, "mcaware_multi_arch_ALL_PREDS_seed23.csv")
all_preds <- list()
for (an in names(ARCH_BUILDERS)) {
  fn <- file.path(OUTDIR, paste0("mcaware_multi_arch_", an, "_PREDICTIONS.csv"))
  if (file.exists(fn)) {
    d <- read.csv(fn)
...
bilstm_fn <- file.path(OUTDIR, "mcaware_BiLSTM_v3b_window_PREDICTIONS.csv")
...
if (length(arch_names) >= 2) {
  ...
  write.csv(mcnemar_df, file.path(OUTDIR, "mcaware_multi_arch_McNEMAR.csv"), row.names=FALSE)
