...
#
# AMACI:
#   v3'te GARAN test setinde gözlenen 15/15 anti-prediktif davranış
...
cat("\n========================================================================\n")
cat("MC-AWARE PROTOTYPE — BiLSTM v3b WINDOW (ADIM I.17 GARAN DOGRULAMA)\n")
...
# --- 1. GARAN veri çekme (Yahoo Finance) — v3 ile AYNI ---
cat("GARAN verisi Yahoo Finance'den cekiliyor...\n")
...
cat(sprintf("Ham GARAN: %d satir\n", nrow(garan_df)))

# --- 2. Teknik göstergeler — v3 ile AYNI ---
garan_df$RSI   <- TTR::RSI(garan_df$Close, n = 14)
macd_vals      <- TTR::MACD(garan_df$Close)
garan_df$MACD  <- macd_vals[, "macd"]
garan_df$EMA12 <- TTR::EMA(garan_df$Close, n = 12)
garan_df$EMA26 <- TTR::EMA(garan_df$Close, n = 26)
stoch_vals     <- TTR::stoch(garan_df[, c("High", "Low", "Close")])
garan_df$SO_K  <- stoch_vals[, "fastK"]
garan_df$SO_D  <- stoch_vals[, "fastD"]
adx_vals       <- TTR::ADX(garan_df[, c("High", "Low", "Close")])
garan_df$ADX   <- adx_vals[, "ADX"]
...
garan_final <- garan_final[28:nrow(garan_final), ]
garan_final <- garan_final %>% drop_na()
cat(sprintf("Final GARAN: %d satir (gunluk, 2018-2026)\n", nrow(garan_final)))
...
cat("Varlik: GARAN.IS (gunluk) | class_weight: balanced\n")
...
cat("\n", strrep("=", 95), "\n", sep = "")
cat("BiLSTM v3b WINDOW — OZET (ADIM I.17 GARAN DOGRULAMA)\n")
cat(strrep("=", 95), "\n", sep = "")
cat(sprintf("\nVarlik: GARAN.IS | Veri: %d gunluk gozlem\n", nrow(garan_final)))
...
if (mean_acc_05 >= 0.50 && flip_wins <= 7) {
  ...
  cat("    Hem v3 hem v3b sonucu noisy; GARAN'da gunluk 3-gun-ileri tahmin\n")
...
# CSV
out1 <- file.path(OUTDIR_PRED, "mcaware_BiLSTM_v3b_GARAN_PREDICTIONS.csv")
out2 <- file.path(OUTDIR_THR, "mcaware_BiLSTM_v3b_GARAN_THRESHOLD_GRID.csv")
out3 <- file.path(OUTDIR_SUM, "mcaware_BiLSTM_v3b_GARAN_OPTIMAL.csv")
out4 <- file.path(OUTDIR_SUM, "mcaware_BiLSTM_v3b_GARAN_SUMMARY.csv")
out5 <- file.path(OUTDIR_DIAG, "mcaware_BiLSTM_v3b_GARAN_YHAT_STATS.csv")
...
cat("\nCSV'leri yukleyince Claude Bolum 10.39 (ADIM I.17 GARAN dogrulamasi) olarak isleyecek.\n")
