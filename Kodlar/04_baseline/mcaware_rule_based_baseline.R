# ===========================================================================
# MC-AWARE — RULE-BASED CLASSIFIER BASELINE (ADIM I.16, OPSIYONEL)
# TÜBİTAK 2209-A — Yürütücü: Mehmet Ali KURT
# Tarih: 23.05.2026
# ---------------------------------------------------------------------------
# AMACI:
#   Klasik rule-based siniflandiricilarin (DT, RIPPER, OneR) THYAO yön
...
cat("\n========================================================================\n")
cat("MC-AWARE — RULE-BASED BASELINE (ADIM I.16)\n")
...
mac <- TTR::MACD(thyao_df$Close); thyao_df$MACD <- mac[,"macd"]
thyao_df$EMA12 <- TTR::EMA(thyao_df$Close, n = 12)
thyao_df$EMA26 <- TTR::EMA(thyao_df$Close, n = 26)
st <- TTR::stoch(thyao_df[,c("High","Low","Close")])
thyao_df$SO_K <- st[,"fastK"]; thyao_df$SO_D <- st[,"fastD"]
thyao_df$ADX  <- TTR::ADX(thyao_df[,c("High","Low","Close")])[,"ADX"]
...
}, error = function(e) tcmb_daily <<- data.frame(Date=as.Date(thyao_df$Date),
                                                  TCMB_Rate=NA_real_))
...
# --- 7. CSV cikti ---
out <- file.path(OUTDIR_SUM, "mcaware_rule_based_RESULTS.csv")
write.csv(results_df, out, row.names = FALSE)
cat(sprintf("\nCSV kaydedildi: %s\n", out))
cat("\nCSV'yi yukleyince Claude Bolum 10.36 (ADIM I.16) olarak isleyecek.\n")
