...
# T�B�TAK 2209-A � Y�r�t�c�: Mehmet Ali KURT
# Tarih: 23.05.2026
# ---------------------------------------------------------------------------
# AMACI:
...
if (m_soft$Acc > naive_acc) {
  ...
  cat("    Mimari cesitliligi anti-prediktif egilimi n�trledi.\n")
  ...
  cat("    Majority Rules baseline anti-prediktif � TUBITAK iddias� icin\n")
  cat("    pozitif kanit.\n")
} else {
  cat("[C] NOTR:\n")
  cat(sprintf("    Ensemble Acc=%.3f, flip=%.3f, Naive=%.3f\n",
              m_soft$Acc, m_soft$Acc_flip, naive_acc))
  cat("    Voting n�tr kaldi - mimariler birbirini iptal ediyor.\n")
}
