...
if (length(arch_files) == 0) {
  ...
  if (file.exists(ens_path)) {
    ...
    # Voting score zaten ensemble'ın temel mekanizması
  ...
  # Monte Carlo simülasyonu: voting score'un ek bilgi sağlayıp sağlamayacağını test et
  set.seed(42)
  n_samples <- 200  # test seti boyutu
  n_archs <- 6
  
  # Her mimari yaklaşık %60 flip accuracy'de çalışıyor (anti-prediktif)
  arch_preds <- matrix(0, nrow = n_samples, ncol = n_archs)
  actual <- rbinom(n_samples, 1, 0.518)  # naive ~ 51.8% UP
  
  for (a in 1:n_archs) {
    # Anti-prediktif: ~40% doğru tahmin
    correct <- rbinom(n_samples, 1, 0.40)
    arch_preds[, a] <- ifelse(correct == 1, actual, 1 - actual)
  }
  
  # Voting score: 6 mimariden kaçı UP dedi (0-6)
  ...
  # ek bilgi sağlayabilir ama ensemble ile eşdeğer
  ...
  # Ortak indeksler üzerinden birleştir ve voting score hesapla
