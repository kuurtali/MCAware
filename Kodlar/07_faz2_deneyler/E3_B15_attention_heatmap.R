################################################################################
# E3/B15 — XAI Attention Heatmap Görselleştirmesi
# BiLSTM+Attention v6'daki attention ağırlıklarını heatmap olarak çiz
################################################################################
...
if (length(attn_cols) > 0) {
  ...
  # --- Ortalama attention ağırlıklarını hesapla ---
  ...
  # --- Heatmap 1: Ortalama attention ağırlıkları bar chart ---
  ...
  # --- Heatmap 2: Lambda x Seed attention değişimi ---
  ...
  cat("UYARI: Attention agirlık sutunlari bulunamadi!\n")
