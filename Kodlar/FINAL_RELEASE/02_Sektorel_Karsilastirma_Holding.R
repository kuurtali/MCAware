...
# ---------------------------------------------------------------------------
# AMACI:
#   Sigorta sektöründe görülen anti-prediktif hatanın, daha iyi hedging
#   (risk dağıtma) yeteneğine sahip olan BIST Holdinglerinde olup 
#   olmadığını test etmek.
#   MIMARI: THYAO deneylerinden seçilen CHAMPION model: 
#           BiLSTM (64 units) + Dropout(0.4) + class_weights (MC koruması)
#
#   Kullanılan Hisseler: KCHOL.IS, SAHOL.IS, DOHOL.IS, ALARK.IS, ENKAI.IS
#   Klasik ML Baseline: Decision Tree.
#
# VERİ KURALI: Veriler quantmod ile internetten çekilir.
# ===========================================================================
...
cat("\n========================================================================\n")
cat("MC-AWARE — BIST-5 HOLDING PORTFOYU TESTI\n")
...
test_stock <- function(ticker, from="2020-01-01", to="2026-03-31") {
  ...
  # CLASS WEIGHTS (MC TUZAGINDAN KURTARAN ANAHTAR)
  ...
  for (sd in SEEDS) {
    ...
    tryCatch({
      ...
      # FIT ILE CLASS_WEIGHT KULLANIMI
      ...
      cat(sprintf("  [DL] seed=%d: Acc=%.3f flip=%.3f naive=%.3f is_MC=%s\n",
                  sd, m$Acc, m$Acc_flip, naive_acc, m$is_MC))
...
TICKERS <- c("KCHOL.IS", "SAHOL.IS", "DOHOL.IS", "ALARK.IS", "ENKAI.IS")
all_results <- list()
...
cat("\nBIST-5 HOLDING TESTI TAMAMLANDI.\n")
